...
export default function FolderTreePanel({ isOpen, onClose }) {
  ...
  const isRootActive = location.pathname === '/dashboard/my-files';
...
function TreeNode({ folder, depth, currentPath, navigate }) {
  ...
  const path = `/dashboard/my-files/${folder.slug}`;
  const isActive = currentPath === path;
  const hasChildren = folder.children?.length > 0;

  return (
    <div>
      <div className={`flex items-center gap-1 rounded-md transition-colors group
        ...
        <button onClick={() => navigate(path)}
          ...
          <span className="text-xs flex-shrink-0">{displayIcon(folder.icon || folder.emoji)}</span>
