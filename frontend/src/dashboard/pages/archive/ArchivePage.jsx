// src/dashboard/pages/archive/ArchivePage.jsx
import { useState, useEffect, useCallback, useRef } from 'react';
import { AnimatePresence } from 'framer-motion';
import { ContextMenuProvider } from '../../components/common/ContextMenu';
import { useArchivedLinks } from './useArchivedLinks';
import ArchiveHeader from '../archive/components/ArchiveHeader';
import ArchiveToolbar from '../archive/components/ArchiveToolbar';
import ArchiveEmpty from '../archive/components/ArchiveEmpty';
...
export default function ArchivePage({
  ...
  const [localViewMode, setLocalViewMode] = useState(() => {
    try { return localStorage.getItem('savlink_archive_view') || 'list'; } catch { return 'list'; }
  });
  ...
  const handleViewModeChange = externalOnViewModeChange || ((mode) => {
    ...
    try { localStorage.setItem('savlink_archive_view', mode); } catch {}
  });
  ...
  const handleMobileDelete = useCallback(async () => {
    ...
    if (!window.confirm(`Permanently delete ${ids.length} link${ids.length > 1 ? 's' : ''}? This cannot be undone.`)) return;
    const r = await LinksService.bulkDelete(ids);
    if (r?.success) { toast.success(`${ids.length} permanently deleted`); refresh(); }
    exitSelectionMode();
  ...
  const handleMobileRestore = useCallback(async () => {
    ...
    let restored = 0;
    for (const id of ids) {
      const r = await LinksService.restoreLink(id);
      if (r?.success) restored++;
    }
    if (restored > 0) { toast.success(`${restored} restored`); refresh(); }
    exitSelectionMode();
  }, [selectedIds, refresh, exitSelectionMode]);
  ...
  const handleBulkRestore = useCallback(async () => {
    const ok = await bulkRestore([...selectedIds]);
    if (ok) clearSelection();
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      ...
      switch (e.key) {
        ...
        case 'r':
          if (focusedIndex >= 0 && links[focusedIndex]) restoreLink(links[focusedIndex].id); break;
      }
    };
    window.addEventListener('keydown', handler);
    return () => window.removeEventListener('keydown', handler);
  }, [isMobile, focusedIndex, links, selectedIds, selectedLink,
    openDetails, toggleSelectById, clearSelection, selectAll, closeDetails, restoreLink]);
          ...
          {!isMobile && (
            <div className="flex-shrink-0 px-6 py-2 border-t border-white/[0.04] bg-[#09090b]/80">
              <div className="flex items-center justify-between text-[11px] text-gray-600">
                <span>
                  {meta.total ?? links.length} archived
        ...
        <AnimatePresence>
          {selectedLink && !isMobile && (
            <LinkDetails
              ...
              onArchive={() => restoreLink(selectedLink.id)}
            />
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {selectedLink && isMobile && !mobileSelectMode && (
            <MobileLinkSheet
              ...
              onPin={() => pinLink(selectedLink.id)}
              onStar={() => starLink(selectedLink.id)}
              onArchive={() => { restoreLink(selectedLink.id); closeDetails(); }}
