// src/dashboard/pages/archive/components/MobileArchiveSelectionBar.jsx
import { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import {
  IconClose, IconTrash, IconDotsVertical,
  IconExternal, IconLink, IconCode,
  IconStar, IconPin, IconCheckCircle,
} from '../../../components/common/Icons';
function haptic(ms = 4) {
  try { navigator?.vibrate?.(ms); } catch {}
}

export default function MobileArchiveSelectionBar({
  selectedCount,
  onClose,
  onRestore,
  onDelete,
      ...
      >
        <div style={{ paddingTop: 'env(safe-area-inset-top, 0px)' }}>
          <div className="flex items-center h-14 px-1">
            ...
            <BarAction label="Restore" onClick={() => { haptic(); onRestore?.(); }}>
...
function BarAction({ label, onClick, children }) {
  return (
    <motion.button
      onClick={onClick}
      whileTap={{ scale: 0.85 }}
      className="w-11 h-11 flex items-center justify-center rounded-full
                 text-gray-300 active:bg-white/[0.1] transition-colors touch-manipulation"
      aria-label={label}
    >
      {children}
    </motion.button>
