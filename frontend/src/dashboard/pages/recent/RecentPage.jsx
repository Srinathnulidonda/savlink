// src/dashboard/pages/recent/RecentPage.jsx
import { useState, useEffect, useCallback, useRef } from 'react';
import { AnimatePresence } from 'framer-motion';
import { ContextMenuProvider } from '../../components/common/ContextMenu';
import { useRecentLinks } from './useRecentLinks';
import RecentHeader from '../recent/components/RecentHeader';
import RecentEmpty from '../recent/components/RecentEmpty';
...
export default function RecentPage({
  ...
  const [localViewMode, setLocalViewMode] = useState(() => {
    try { return localStorage.getItem('savlink_recent_view') || 'list'; } catch { return 'list'; }
  });
  ...
  const handleViewModeChange = externalOnViewModeChange || ((mode) => {
    ...
    try { localStorage.setItem('savlink_recent_view', mode); } catch {}
  });
          ...
          {!isMobile && (
            <div className="flex-shrink-0 px-6 py-2 border-t border-white/[0.04] bg-[#09090b]/80">
              <div className="flex items-center justify-between text-[11px] text-gray-600">
                <span>
                  {meta.total ?? links.length} recent
