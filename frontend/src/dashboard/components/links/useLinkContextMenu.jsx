...
export function useLinkContextMenu({ onPin, onStar, onArchive, onDelete, onSelect }) {
    const getMenuItems = useCallback((link) => {
        ...
        return [
            ...
            {
                id: 'star',
                label: link.starred ? 'Remove from favorites' : 'Add to favorites',
                shortcut: 'S',
                active: !!link.starred,
                icon: (
                    <svg className="w-4 h-4" fill={link.starred ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
                    </svg>
                ),
                action: () => onStar?.(),
            },
            // ── Pin = sidebar / quick-access ───────────
            {
                id: 'pin',
                label: link.pinned ? 'Unpin' : 'Pin to quick access',
                shortcut: 'P',
                active: !!link.pinned,
                icon: (
                    <svg className="w-4 h-4" viewBox="0 0 16 16"
                        fill={link.pinned ? 'currentColor' : 'none'}
                        stroke={link.pinned ? 'none' : 'currentColor'}
                        strokeWidth={1}>
