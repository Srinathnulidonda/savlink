// src/dashboard/views/Collection/useCollectionData.js
import { useState, useEffect, useCallback } from 'react';
import { LinksService } from '../../../services/links.service';
import { FoldersService } from '../../../services/folders.service';

export default function useCollectionData({ collectionId, searchQuery, sortBy, filters }) {
    ...
    const fetchData = useCallback(async (reset = false) => {
        try {
            ...
            if (!collection || reset) {
                const collectionResult = await FoldersService.getFolder(collectionId);
                if (collectionResult?.success) {
                    setCollection(collectionResult.data.folder);
                }
            }
    ...
    }, [collectionId, searchQuery, sortBy, filters, cursor, collection]);
    ...
    useEffect(() => {
        setCursor(null);
        fetchData(true);
    }, [collectionId, searchQuery, sortBy, filters]);
